    ...
    ([
        ...
        if (!$Number(' 0o1') || !$Number('0b1') || $Number('+0x1')) {
          $Number = function Number(value) {
            ...
            return that instanceof $Number
              ...
              && (BROKEN_COF ? fails(function() { proto.valueOf.call(that); }) : cof(that) != NUMBER) ? inheritIfRequired(new Base(toNumber(it)), that, $Number) : toNumber(it);
          };
      ...
      function(module, exports, __webpack_require__) {
        ...
        var ctx = __webpack_require__(18),
          ...
          aFunction = __webpack_require__(19),
          forOf = __webpack_require__(198),
          isIterable = __webpack_require__(246),
          $iterCreate = __webpack_require__(136),
          step = __webpack_require__(184),
          isObject = __webpack_require__(11),
          toIObject = __webpack_require__(30),
...
/***/ function(module, exports, __webpack_require__) {
  ...
  var IDKEY_MAX_CACHE = 1;
  var KEYVALE_MAX_CACHE = 20;
  var LOG_MAX_CACHE = 50;

  // 接口调用频率限制
  var SUBMIT_DISTANCE = 50;
  var KEYVALUE_DISTANCE = 50;
  var IDKEY_DISTANCE = 50;
  var LOG_DISTANCE = 50;
  var SPEED_DISTANCE = 500;
  var SLOW_DISTANCE = 500;

  // 同类型上报限制
  var ThirdSameErrorReportTimes = 3;
  var FrameworkSameErrorReportTimes = 3;
  var SlowSameReportTimes = 3;
  var MaxErrorTypes = 50;
  var MaxSlowTypes = 50;

  var KeyValueReportCache = [];
  var IDKeyReportCache = [];
  var SystemLogCache = [];
  var KeyValueInvokeTime = 0;
  var IDKeyInvokeTime = 0;
  var LogInvokeTime = 0;
  var SubmitInvokeTime = 0;
  var SlowInvokeTime = 0;
  var SpeedInvokeTime = {};
  var ErrorReportMap = {};
  var SlowReportMap = {};
  ...
  function invoke() {
    ...
    onBridgeReady(function () {
      WeixinJSBridge.invoke.apply(WeixinJSBridge, _arguments);
    });
  }
  ...
  function publish() {
    ...
    onBridgeReady(function () {
      WeixinJSBridge.publish.apply(WeixinJSBridge, _arguments2);
    });
  }
  ...
  function getLibVersion() {
    if (typeof wx != 'undefined') {
      return wx.version && wx.version.version || '';
    }
    return '';
  }

  function submitReportKeyValue() {
    if (!KeyValueReportCache || KeyValueReportCache.length <= 0) return;
    invoke("reportKeyValue", {
      dataArray: KeyValueReportCache
    });
    KeyValueReportCache = [];
  }

  function submitReportIDKey() {
    if (!IDKeyReportCache || IDKeyReportCache.length <= 0) return;
    invoke("reportIDKey", {
      dataArray: IDKeyReportCache
    });
    IDKeyReportCache = [];
  }

  function submitSystemLog() {
    if (!SystemLogCache || SystemLogCache.length <= 0) return;
    invoke("systemLog", {
      dataArray: SystemLogCache
    });
    SystemLogCache = [];
  }
  ...
  var isIOS = getPlatform() === 'ios';
  var isDevTools = getPlatform() === 'devtools';
  ...
  var Reporter = {
    surroundThirdByTryCatch: function surroundThirdByTryCatch(fn, extend) {
      var route = '';
      if (__route__) {
        route = 'at ' + __route__ + ' ' + __method__ + ' function;';
      }
    ...
    slowReport: function slowReport(_ref) {
      var key = _ref.key,
          cost = _ref.cost,
          extend = _ref.extend,
          force = _ref.force;

      var type = _Constants.SlowValueType[key];
      var now = Date.now();
      if (!type) return;
      if (!force && now - SlowInvokeTime < SLOW_DISTANCE) return;

      if (Object.keys(SlowReportMap).length > MaxSlowTypes) {
        // 这个小程序慢的有一笔了
        return;
      }

      if (!SlowReportMap[extend]) {
        SlowReportMap[extend] = 0;
      }
      SlowReportMap[extend]++;
      // 避免频繁上报
      if (SlowReportMap[extend] > SlowSameReportTimes) return;

      SlowInvokeTime = now;
      var value = cost + ',' + encodeURIComponent(extend) + ',' + type;
    ...
    speedReport: function speedReport(_ref2) {
      var key = _ref2.key,
          data = _ref2.data,
          timeMark = _ref2.timeMark,
          force = _ref2.force;

      var direction = _Constants.SpeedValueType[key];
      var now = Date.now();
      var dataSize = 0;
      var nativeTime = timeMark.nativeTime;

      if (!direction) return;
      if (!force && now - (SpeedInvokeTime[direction] || 0) < SPEED_DISTANCE) return;
      if (!timeMark.startTime || !timeMark.endTime) return;
      if ((direction == 1 || direction == 2) && !nativeTime) return;

      if (data) {
        dataSize = JSON.stringify(data).length;
      }

      SpeedInvokeTime[direction] = now;
      var value = direction + ',' + timeMark.startTime + ',' + nativeTime + ',' + nativeTime + ',' + timeMark.endTime + ',' + dataSize;
      Reporter.reportKeyValue({
        key: "Speed",
        value: value,
        force: true
    ...
    reportKeyValue: function reportKeyValue(_ref3) {
      var key = _ref3.key,
          value = _ref3.value,
          force = _ref3.force;

      if (!_Constants.KeyValueType[key]) return;
      //小于调用间隔抛弃
      if (!force && Date.now() - KeyValueInvokeTime < KEYVALUE_DISTANCE) return;
      KeyValueInvokeTime = Date.now();

      KeyValueReportCache.push({
        key: _Constants.KeyValueType[key],
        value: value
      });
      if (KeyValueReportCache.length >= KEYVALE_MAX_CACHE) {
        submitReportKeyValue();
      }
    },
    reportIDKey: function reportIDKey(_ref4) {
      var id = _ref4.id,
          key = _ref4.key,
          force = _ref4.force;

      if (!_Constants.IDKeyType[key]) return;
      //小于调用间隔抛弃
      if (!force && Date.now() - IDKeyInvokeTime < IDKEY_DISTANCE) return;
      IDKeyInvokeTime = Date.now();

      IDKeyReportCache.push({
        id: id ? id : isIOS ? '356' : '358',
        key: _Constants.IDKeyType[key],
        value: 1
      });

      if (IDKeyReportCache.length >= IDKEY_MAX_CACHE) {
        submitReportIDKey();
      }
    },
    thirdErrorReport: function thirdErrorReport(_ref5) {
    ...
    errorReport: function errorReport(_ref6) {
      var key = _ref6.key,
          error = _ref6.error,
          extend = _ref6.extend;

      if (!_Constants.ErrorType[key]) return;
      var msg = extend ? error.message + ';' + extend : error.message;
      var errMsg = key + '\n' + msg + '\n' + error.stack;
      console.error(errMsg);
      if (typeof window != 'undefined' && typeof window.__webviewId__ != 'undefined') {
      ...
      if (Object.keys(ErrorReportMap).length > MaxErrorTypes) {
        //竟然有 50种 不同的error 这个小程序是专门来 throw error 的么
        return;
      }

      var value = _Constants.ErrorType[key] + ',' + error.name + ',' + encodeURIComponent(msg) + ',' + encodeURIComponent(error.stack) + ',' + encodeURIComponent(getLibVersion());
      if (!ErrorReportMap[value]) {
        ErrorReportMap[value] = 0;
      }
      ErrorReportMap[value]++;
      if (key === 'thirdScriptError' && ErrorReportMap[value] > ThirdSameErrorReportTimes || ErrorReportMap[value] > FrameworkSameErrorReportTimes) {
        return;
      }
    ...
    log: function log(msg, force) {
      ...
      //小于调用间隔抛弃
      if (!force && Date.now() - LogInvokeTime < LOG_DISTANCE) return;
      LogInvokeTime = Date.now();

      SystemLogCache.push(msg + "");

      if (SystemLogCache.length >= LOG_MAX_CACHE) {
        submitSystemLog();
      }
    },
    submit: function submit() {
      //小于调用间隔抛弃
      if (Date.now() - SubmitInvokeTime < SUBMIT_DISTANCE) return;
      SubmitInvokeTime = Date.now();

      submitReportIDKey();
      submitReportKeyValue();
      submitSystemLog();
    },
    registerErrorListener: function registerErrorListener(func) {
      if (typeof func === 'function') {
        errorCallbackFunc = func;
      }
    },
    unRegisterErrorListener: function unRegisterErrorListener() {
      errorCallbackFunc = function errorCallbackFunc() {};
    },
    triggerErrorMessage: function triggerErrorMessage(msg) {
      ...
      // 使用异步调用，避免同步错误出现的 Max Stack Size 的问题
      setTimeout(function () {
        try {
          errorCallbackFunc(msg);
        } catch (e) {
          // onError 里面有 Error
  ...
  /** surround all report function by try catch**/
  var exportApi = {};
  function surroundByTryCatchFactory(fn) {
    return function () {
      try {
  ...
  function surroundByTryCatch(key) {
    exportApi.__defineGetter__(key, function () {
      return surroundByTryCatchFactory(Reporter[key]);
    });
  }
  ...
  exportApi.__defineSetter__('__route__', function (value) {
    __route__ = value;
  });

  exportApi.__defineSetter__('__method__', function (value) {
    __method__ = value;
  });
...
/***/ function(module, exports, __webpack_require__) {
  ...
  var _BaseMethods = __webpack_require__(1);

  var _Canvas = __webpack_require__(13);

  var _Utils = __webpack_require__(2);

  var onCanvasInsertCb = void 0;
  var onDataPushCb = void 0;
  var canvasInsertRes = void 0;
  var dataPushRes = void 0;
  var onLoaded = void 0;

  var platform = (0, _Utils.getPlatform)();

  var executeFunction = function executeFunction() {
    if (canvasInsertRes && onCanvasInsertCb) {
      if (!onLoaded) {
        onCanvasInsertCb(canvasInsertRes);
        onLoaded = true;
      }

      if (dataPushRes && onDataPushCb) {
        onDataPushCb(dataPushRes);
      }
    }
  };

  var wx = {
    createWidgetContext: _Canvas.createWidgetContext,
    onCanvasInsert: function onCanvasInsert(cb) {
      onCanvasInsertCb = cb;
      executeFunction();
    },
    onDataPush: function onDataPush(cb) {
      onDataPushCb = cb;
      executeFunction();
    },
    getSystemInfo: function getSystemInfo(args) {
      (0, _BaseMethods.invokeMethod)('getSystemInfo', args, {
        beforeSuccess: function beforeSuccess(res) {
          res.platform = platform;
        }
      });
    },
    getSystemInfoSync: function getSystemInfoSync(args) {
      var systemInfo = {};
      (0, _BaseMethods.invokeMethod)('getSystemInfo', {}, {
  ...
  (0, _BaseMethods.onMethod)('onCanvasInsert', function (res) {
    canvasInsertRes = res;
    executeFunction();
  });

  (0, _BaseMethods.onMethod)('onDataPush', function (res) {
    dataPushRes = res;
    executeFunction();
  });
...
/***/ function(module, exports, __webpack_require__) {
  ...
  exports.Context = exports.notifyCurrentRoutetoContext = undefined;
  ...
  var _PredefinedColor = __webpack_require__(14);

  function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }

  var contextMethods = ['scale', 'rotate', 'translate', 'save', 'restore'];
  var drawMethods = ['drawImage', 'fillText', 'fill', 'stroke', 'fillRect', 'strokeRect', 'clearRect'];
  var pathMethods = ['beginPath', 'moveTo', 'lineTo', 'rect', 'arc', 'quadraticCurveTo', 'bezierCurveTo', 'closePath'];
  var styleMethods = ['setFillStyle', 'setTextAlign', 'setStrokeStyle', 'setGlobalAlpha', 'setShadow', 'setFontSize', 'setLineCap', 'setLineJoin', 'setLineWidth', 'setMiterLimit'];

  var currentRoute = '';

  function notifyCurrentRoutetoContext(cr) {
    currentRoute = cr;
  }
  ...
  function formatColor(color) {
    ...
    if ((result = /^#([0-9|A-F|a-f]{6})$/.exec(color)) != null) {
      var r = parseInt(result[1].slice(0, 2), 16);
      var g = parseInt(result[1].slice(2, 4), 16);
      var b = parseInt(result[1].slice(4), 16);
      return [r, g, b, 255];
    }
    if ((result = /^rgb\((.+)\)$/.exec(color)) != null) {
      return result[1].split(',').map(function (item) {
        return parseInt(item.trim());
      }).concat(255);
    }
    if ((result = /^rgba\((.+)\)$/.exec(color)) != null) {
      return result[1].split(',').map(function (item, index) {
        if (index == 3) {
          return Math.floor(255 * parseFloat(item.trim()));
        } else {
          return parseInt(item.trim());
        }
      });
    ...
    if (_PredefinedColor.predefinedColor.hasOwnProperty(lowerColor)) {
      result = /^#([0-9|A-F|a-f]{6})$/.exec(_PredefinedColor.predefinedColor[lowerColor]);
      var r = parseInt(result[1].slice(0, 2), 16);
      var g = parseInt(result[1].slice(2, 4), 16);
      var b = parseInt(result[1].slice(4), 16);
      return [r, g, b, 255];
    } else {
      console.group('非法颜色: ' + color);
      console.error('不支持颜色：' + color);
      console.groupEnd();
    }
  }
  ...
  var Gradient = function () {
    ...
    _createClass(Gradient, [{
      ...
      value: function addColorStop(stop, color) {
        this.colorStop.push([stop, formatColor(color)]);
      }
    }]);
  ...
  var Context = function () {
    ...
    _createClass(Context, [{
      ...
      value: function createLinearGradient(x, y, x1, y1) {
        return new Gradient('linear', [x, y, x1, y1]);
      }
    }, {
      key: 'createCircularGradient',
      value: function createCircularGradient(x, y, r) {
        // for android client, only support circular gradient
        // TODO(hulkhu): replace radial to circular
        return new Gradient('radial', [x, y, r]);
      }
    }]);
  ...
  ;[].concat(contextMethods, drawMethods).forEach(function (method) {
    if (method == 'fill' || method == 'stroke') {
      ...
      Context.prototype[method] = function (x, y, width, height) {
        this.actions.push({
          method: 'fillPath',
          data: [{
            method: 'rect',
      ...
      Context.prototype[method] = function (text, x, y) {
        this.actions.push({
          method: method,
          data: [text.toString(), x, y]
        });
      };
    } else if (method == 'drawImage') {
      Context.prototype[method] = function (imagePath, x, y, width, height) {
        if ((0, _Utils.getPlatform)() != 'devtools' && !/wxfile:\/\//.test(imagePath)) {
          imagePath = (0, _Utils.getRealRoute)(currentRoute, imagePath).replace(/.html$/, '');
          // console.log(`计算后的绝对路径${imagePath}`);
        } else {
            // console.log(`不需要计算的路径${imagePath}`);
          }

        if (isNumber(width) && isNumber(height)) {
          data = [imagePath, x, y, width, height];
        } else {
          data = [imagePath, x, y];
        }
  ...
  styleMethods.forEach(function (method) {
    if (method == 'setFillStyle' || method == 'setStrokeStyle') {
      Context.prototype[method] = function () {
        var arg = arguments[0];
        ...
        if (typeof arg == 'string') {
          this.actions.push({
            method: method,
            data: ['normal', formatColor(arg)]
          });
          // 渐变
        } else if ((typeof arg === 'undefined' ? 'undefined' : _typeof(arg)) == 'object' && arg instanceof Gradient) {
          this.actions.push({
            method: method,
            data: [arg.type, arg.data, arg.colorStop]
          });
        }
      };
    } else if (method === 'setGlobalAlpha') {
      Context.prototype[method] = function () {
  ...
  exports.notifyCurrentRoutetoContext = notifyCurrentRoutetoContext;
...
/***/ function(module, exports, __webpack_require__) {
  ...
  exports.createWidgetContext = exports.createCanvasContext = exports.createContext = exports.canvasToTempFilePath = exports.drawCanvas = exports.notifyWebviewIdtoCanvas = exports.clearOldWebviewCanvas = exports.canvasInfo = undefined;
  ...
  var currentWebviewId = 0;
  var canvasInfo = {};

  function concatId(webviewId, canvasId) {
  ...
  WeixinJSBridge.subscribe('canvasInsert', function (_ref, webviewId) {
    var canvasId = _ref.canvasId,
        canvasNumber = _ref.canvasNumber,
        data = _ref.data;

    var id = concatId(currentWebviewId, canvasId);
    ...
    canvas[id] = canvas[id] || canvasNumber;

    // console.log(`webview ${currentWebviewId} 中的 canvas ${canvasId} 已经创建, 序号${canvasNumber}`);

    if (Array.isArray(deferred[id])) {
      // console.log(`webview ${currentWebviewId} 中的 canvas ${canvasId} 序号 ${canvasNumber} 调用了被延迟的drawCanvas`);

      deferred[id].forEach(function (item) {
        _invokeDrawCanvas(canvasNumber, item.actions, item.reserve, item.success, item.fail, item.complete);
      });
  ...
  WeixinJSBridge.subscribe('canvasRemove', function (_ref2, webviewId) {
    var canvasId = _ref2.canvasId;

    var id = concatId(currentWebviewId, canvasId);

    if (canvas[id]) delete canvas[id];

    // console.log(`webview ${currentWebviewId} 中的 canvas ${canvasId} 已经移除`);
  });

  function clearOldWebviewCanvas() {
    for (var k in canvas) {
      if (k.indexOf(currentWebviewId + 'canvas') == 0) {
        var canvasNumber = canvas[k];

        delete canvas[k];

        // console.log(`删除 webview ${currentWebviewId} 中的 ${canvasNumber}`);
      }
    }
  }

  function notifyWebviewIdtoCanvas(webviewId) {
    currentWebviewId = webviewId;
    // console.log(`canvas正在记录, 路由至 webview ${currentWebviewId}`);
  }

  function _invokeDrawCanvas(canvasNumber, actions) {
    var reserve = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
    var success = arguments[3];
    var fail = arguments[4];
    var complete = arguments[5];

    var platform = (0, _Utils.getPlatform)();

    if (platform == 'ios' || platform == 'android') {
      WeixinJSBridge.invoke('drawCanvas', {
        canvasId: canvasNumber,
        reserve: reserve,
        actions: actions
      }, function (res) {
        if (res.errMsg && /ok/.test(res.errMsg)) {
      ...
      // console.log(`发布canvas${canvasNumber}actionsChanged事件到webview`);

      WeixinJSBridge.publish('canvas' + canvasNumber + 'actionsChanged', { actions: actions, reserve: reserve });
    }
  }

  function drawCanvas(_ref3) {
    var isWidgetCanvas = _ref3.isWidgetCanvas,
        canvasId = _ref3.canvasId,
    ...
    if (typeof canvas[id] === 'number') {
      var canvasNumber = canvas[id];

      // console.log(`webview ${currentWebviewId} 中的 canvas ${canvasId} 已经就绪, 序号${canvasNumber}, 直接调用drawCanvas`);
      _invokeDrawCanvas(canvasNumber, actions, reserve, success, fail, complete);
    } else {
      // console.log(`webview ${currentWebviewId} 中的 canvas ${canvasId} 没有就绪, drawCanvas的调用被延迟`);

      deferred[id] = deferred[id] || [];

      deferred[id] = deferred[id].concat({
        actions: actions,
        reserve: reserve,
        success: success,
        fail: fail,
  ...
  function invokeToTempFilePath(args) {
    var platform = (0, _Utils.getPlatform)();
    // 如果是客户端直接调用
    if (platform === 'ios' || platform === 'android') {
      (0, _BaseMethods.invokeMethod)('canvasToTempFilePath', args);

      // 如果是开发者工具，需要由 webview 传递 base64 数据回来，然后调用 base64ToTempFilePath
    } else {
      WeixinJSBridge.subscribe('onCanvasToDataUrl_' + args.canvasId, function (_ref4) {
        var dataUrl = _ref4.dataUrl;

        (0, _BaseMethods.invokeMethod)('base64ToTempFilePath', (0, _Utils.assign)({
          base64Data: dataUrl
        }, args), {
      ...
      (0, _BaseMethods.publish)('invokeCanvasToDataUrl_' + args.canvasId, {
  ...
  function canvasToTempFilePath(args) {
    ...
    var id = concatId(currentWebviewId, args.canvasId);

    if (typeof canvas[id] === 'number') {
      args.canvasId = canvas[id];
      invokeToTempFilePath(args);
    } else {
      // canvasId 不合法，可能是错误的 canvasId 也可能是 canvas 还没插入
      var res = {
  ...
  var createWidgetContext = function createWidgetContext() {
    var ctx = new _Context.Context();
    ctx.isWidgetCanvas = true;
    return ctx;
  };

  var createCanvasContext = function createCanvasContext(canvasId) {
    return new _Context.Context(canvasId);
  };

  exports.canvasInfo = canvasInfo;
  exports.clearOldWebviewCanvas = clearOldWebviewCanvas;
  exports.notifyWebviewIdtoCanvas = notifyWebviewIdtoCanvas;
  ...
  exports.createWidgetContext = createWidgetContext;
...
/***/ function(module, exports) {
  ...
  exports.predefinedColor = predefinedColor;

/***/ }
/******/ ]);
var __widgetEngine__ =
/******/ (function(modules) { // webpackBootstrap
/******/  // The module cache
/******/  var installedModules = {};
...
/***/ function(module, exports, __webpack_require__) {
  ...
  Object.defineProperty(exports, 'Widget', {
    enumerable: true,
    get: function get() {
      return _widget.WidgetHolder;
    }
  });
  Object.defineProperty(exports, 'getWidget', {
    enumerable: true,
    get: function get() {
      return _widget.getWidget;
    }
  });


  if (typeof logxx === 'function') {
    // eslint-disable-next-line no-undef
    logxx('widget engine start');
  }
...
/***/ function(module, exports, __webpack_require__) {
  ...
  exports.getWidget = exports.WidgetHolder = undefined;
  ...
  //   cacheKey
  // }
  var privateKeyword = ['__context__', 'getContext'];

  var lifeCycleMethod = ['onLoad', 'onDataPush'];

  var isLifeCycleMethod = function isLifeCycleMethod(key) {
    for (var i = 0; i < lifeCycleMethod.length; ++i) {
      if (lifeCycleMethod[i] === key) {
        return true;
      }
    }
    return false;
  ...
  var isPrivateKeyword = function isPrivateKeyword(key) {
    for (var i = 0; i < privateKeyword.length; ++i) {
      if (privateKeyword[i] === key) {
        return true;
      }
    }
    return false;
  ...
  var Widget = function () {
    function Widget(options) {
      ...
      var _loop = function _loop(key) {
        if (isPrivateKeyword(key)) {
          (0, _util.warn)('关键字保护', 'Widget\'s ' + key + ' is write-protected');
        } else if (!isLifeCycleMethod(key)) {
          if ((0, _util.getDataType)(options[key]) === 'Function') {
            _this[key] = function () {
      ...
      // bind other field on this
      for (var key in options) {
        _loop(key);
      }

      // eslint-disable-next-line no-undef
      this.__context__ = wx.createWidgetContext();

      lifeCycleMethod.forEach(function (key) {
        _this[key] = function (lifeCycleOptions) {
          var method = (options[key] || _util.noop).bind(this);

          method(lifeCycleOptions);
        }.bind(_this);
      });

      // eslint-disable-next-line no-undef
      wx.onCanvasInsert(this.onLoad);
      // eslint-disable-next-line no-undef
      wx.onDataPush(this.onDataPush);
    }
  ...
  var WidgetHolder = exports.WidgetHolder = function WidgetHolder(options) {
    widgetInstance = new Widget(options);
  };
...
/***/ function(module, exports) {
  ...
  var warn = exports.warn = function warn(title, descript) {
    console.group(new Date() + ' ' + title);
    console.warn(descript);
    console.groupEnd();
  };

/***/ }
/******/ ]);
var Widget=__widgetEngine__.Widget;var getWidget=__widgetEngine__.getWidget;
(function(){
  ...
  var _require = function(modId){
    ...
    return function(requireId){
      ...
      var realFilepath = [],
        filepaths = (modDir+"/"+requireId).split("/")
      ...
      for (var i = 0, len = filepaths.length; i < len; ++i){
        ...
        if (filepath == '..'){
          if (realFilepath.length == 0){
            throw new Error("can't find module : " + requireId)
          }
          realFilepath.pop()
        }else if (i+1 < len && filepaths[i+1] == '..'){
          i++
        }else{
          realFilepath.push(filepath)
        }
      }
  ...
  require = function (modId) {
    ...
    if (mod.status === MODULE_STATUS_UNLOAD){
      ...
      mod.exports = module.exports || ret
...
wx.version = { updateTime: '2017.4.13 17:17:56', info: '', version: 'develop' };